      ...
      <div class="scroll-view__header" ref="scrollViewHeaderRef" :style="_headerStyle" v-if="refreshEnabled">
        <div class="pull-header">
          <div class="pull-header-icon">
            <i
              :class="['iconfont-bpf-m', 'icon-m-arrowdown', 'ic-arrow', move.state == 0 ? 'up' : 'down']"
              v-if="move.state < 2"
            ></i>
...
<script>
import { _formatDate, uniqueKey } from '@/utils/common'
...
export default {
  ...
  computed: {
    ...
    _bodyStyle() {
      return {
        ...
        marginTop: `-${this.refreshEnabled ? this.refreshThreshold : 0}px`
      }
    },
    _headerStyle() {
      return {
  ...
  methods: {
    ...
    handleScroll() {
      if (this.move.distance > 0) return
     ...
     * */
    onTouchMove(e) {
      if (!this._enablePull || e.targetTouches.length > 1) return
