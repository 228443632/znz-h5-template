...
<template>
  <div class="page-index" ref="container">
    ...
    <div class="page__body" ref="pageBody">
      <g-scroll-view-simple :height="`${height}px`" @on-scroll-to-bottom="onScrollToBottom" ref="scrollView">
          ...
          <template v-else>
            <template v-if="list.length">
              <record-list
                v-for="item in 30"
                :key="item"
                @click.stop.native="$router.push(`/out-patient/record-detail/${item}`)"
              ></record-list>
              <g-loading-end
    ...
    <van-popup v-model="visible.filter" position="top" :style="{ height: 'calc(100% - 1rem)' }">
      <filter-view :visible.sync="visible.filter"></filter-view>
    </van-popup>
  </div>
</template>
